import { VehicleType, SuggestedRoute, WeeklyVehicleAvailability, Stop } from './types';
...
export async function generateSuggestedRoutes(
  ...
  if (availability.cars > 0) {
    ...
    const isDifferentOrder = reordered.some((s, i) => s.id !== currentStops[i].id);
    if (isDifferentOrder) {
      variants.push({
        stops: reordered,
        vehicleType: 'car',
  ...
  if (availability.motorcycles > 0) {
    ...
    const motoStops = middle.length > 0 ? [start, middle[0], end] : [start, end];
    variants.push({
      stops: motoStops,
      vehicleType: 'motorcycle',
      optimizationType: 'fastest',
      name: 'Route C – Motorcycle Express',
      description: middle.length > 0
        ? `${motoStops.length}-stop express route for a motorcycle.`
        : `Direct route for a motorcycle.`,
    });
  }

  const results = await Promise.allSettled(variants.map((v) => fetchRoute(v.stops)));
